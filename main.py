...
async def call_ask_news(query, summariser_fn=call_claude):
    ask = AskNewsSDK(client_id=ASK_NEWS_CLIENT_ID, client_secret=ASK_NEWS_CLIENT_SECRET, timeout=60)
...
COST_DICT = {"claude-3-5-sonnet-20241022": {"input": 3.0/1000000, "output": 15.0/1000000}, "o1-preview": {"input": 15/1000000, "output": 60/1000000}}
...
def main():
    ...
    results = asyncio.run(ensemble_async(get_prediction, ids, num_agents=2 if DEBUG_MODE else 32, aggregate_fn=aggregate_by_consensus, news_fn=call_ask_news, model_fn=call_claude, prompt_template=PROMPT_TEMPLATE))
    logger.info(results)
    if DEBUG_MODE:
        logger.info("WARNING: DEBUG MODE ENABLED. PREDICTIONS WILL NOT BE SUBMITTED.")
